<template>
  <div class="mx-auto mt-8 max-w-3xl rounded-2xl border border-cyan-100/14 bg-[#111722]/88 shadow-[0_30px_120px_rgba(30,130,255,.18)] backdrop-blur-xl">
    <div class="p-3">
      <textarea
        v-model="goal"
        class="h-24 w-full resize-none rounded-xl bg-transparent px-3 py-3 text-sm text-white outline-none placeholder:text-white/30"
    ...
    <div class="flex items-center justify-between border-t border-cyan-100/10 px-4 pb-3">
      ...
      <button
        :disabled="loading"
        class="grid size-9 place-items-center rounded-full text-[#061018] transition"
        :class="loading ? 'bg-white/30' : 'bg-cyan-100 hover:bg-white'"
        @click="submit"
      >
      ...
      <div class="p-5">
        <div class="rounded-xl border border-white/8 bg-black/16 p-4">
          ...
          <label class="mb-4 block">
            ...
            <input v-model="chatForm.baseUrl" class="h-11 w-full rounded-lg border border-white/8 bg-black px-3 text-sm text-white outline-none focus:border-cyan-400/60" />
          </label>
          <label class="mb-4 block">
          ...
          <label class="mb-5 block">
            ...
            <input v-model="chatForm.model" class="h-11 w-full rounded-lg border border-white/8 bg-black px-3 text-sm text-white outline-none focus:border-cyan-400/60" placeholder="deepseek-chat" />
          </label>
...
function saveApiSettings() {
  ...
  aiService.saveProviders({ provider: 'chat', baseUrl: chatForm.baseUrl, apiKey: chatForm.apiKey, model: chatForm.model })
    .then(() => { apiStatus.value = t('已保存', 'Saved.') })
...
async function testConnection() {
  ...
  try {
    ...
    apiStatus.value = res?.content ? t('连接成功', 'Connection successful.') : t('连接成功，无返回', 'Connected, no reply.')
  } catch (err: any) {
...
async function submit() {
  ...
  try {
    const res: any = await aiService.chat(applyApiConfig('chat', {
      ...
      messages: [
        { role: 'system', content: settingsStore.isZh
