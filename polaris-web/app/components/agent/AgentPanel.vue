<template>
  <aside class="flex h-full w-full flex-col bg-[#07111f]/95 backdrop-blur-xl">
    <div class="border-b border-cyan-100/10 p-4">
      <div class="flex items-center gap-3">
        <div class="grid size-9 place-items-center overflow-hidden rounded-full border border-cyan-100/25 bg-cyan-100/10">
          <img src="/brand/polaris-logo.png" alt="Polaris" class="size-9 scale-[2.4] object-cover" />
        </div>
        <div>
          <h2 class="text-sm font-semibold text-white">Polaris Copilot</h2>
          <p class="text-xs text-white/42">{{ t('北极星创作导航官', 'Creative navigation console') }}</p>
        </div>
      </div>
    </div>

    <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
      <section class="rounded-xl border border-cyan-100/12 bg-white/6 p-4">
        <p class="text-xs uppercase tracking-[0.16em] text-cyan-100/55">{{ t('当前建议', 'Current Suggestion') }}</p>
        <p class="mt-3 text-sm leading-6 text-white/72">
          {{ t('检测到当前路线缺少输出节点。建议将 Prompt、图像生成和视频生成连接成一条可执行航线。', 'This route is missing an output node. Connect Prompt, Image and Video nodes into an executable route.') }}
        </p>
        <button class="mt-3 rounded-lg border border-cyan-100/18 px-3 py-2 text-xs text-cyan-50 transition hover:border-cyan-100/45 hover:bg-cyan-100/10" @click="buildWorkflowFromMessage(t('将当前目标整理成 Prompt -> Image -> Video -> Render 的星图航线。', 'Build a Prompt -> Image -> Video -> Render star-map route.'))">
          {{ t('应用建议', 'Apply') }}
        </button>
      </section>
    ...
    <div class="border-t border-cyan-100/10 p-4">
      ...
      <div class="mt-4 rounded-2xl border border-cyan-100/12 bg-black/35 p-2">
        <textarea
          ...
          :placeholder="t('告诉我你的创作目标，或输入 @ 引用素材', 'Describe your goal, or type @ to reference assets')"
...
import { Boxes, Clapperboard, ImageIcon, Send, Sparkles, WandSparkles, Workflow } from 'lucide-vue-next'
...
const messages = ref<AgentMessage[]>([
  {
    id: uid('msg'),
    role: 'assistant',
    content: t('告诉我你的北极星目标，我可以把它拆成 Prompt、图像、视频和分镜路线，并连接到画布。', 'Tell me your north-star goal. I can create linked prompt, image, video and storyboard routes on the canvas.'),
    timestamp: Date.now()
  }
])
...
function handleQuickAction(action: QuickActionKey) {
  if (action === 'video') {
    addUserMessage(t('创建视频生成节点', 'Create a video generation node'))
    createVideoWaypoint()
    addAssistantMessage(t('已添加视频节点。把它连接到图像或 Prompt 节点，就能形成生成路线。', 'Video node added. Connect it to an image or prompt node to form a generation route.'), true)
    return
  }
  if (action === 'image') {
    addUserMessage(t('创建图像生成节点', 'Create an image generation node'))
    createImageWaypoint()
    addAssistantMessage(t('已添加图像节点。它可以接收 Prompt，并继续输入到视频生成。', 'Image node added. It can receive a prompt and feed into video generation.'), true)
    return
  }
  addUserMessage(t('创建分镜路线', 'Create a storyboard route'))
  createStoryboardRoute()
  addAssistantMessage(t('已添加分镜路线，并创建可见连接；Workflow 节点也已准备好。', 'Storyboard route added with visible links. Workflow nodes are prepared as well.'), true)
}
...
function createImageWaypoint(content = t('描述图像生成、参考图或编辑要求。', 'Describe image generation, references or editing instructions.')) {
